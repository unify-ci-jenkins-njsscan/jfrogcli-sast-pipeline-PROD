pipeline {
  ...
  environment {
    ...
    SAST_PROJECT_DIR = "${env.WORKSPACE}/dvja" // entire repo - "${env.WORKSPACE}" 
      ...
      steps {
        dir("${env.SAST_PROJECT_DIR}") {
          sh '''
            ...
            "$JFROG_CLI_PATH" aud --sast --format=sarif . > dvja_jfrog_sast.sarif || true
          '''
        }
      }
    }
    ...
    stage('Display SARIF Output') {
      steps {
        sh '''
          ...
          head -n 50 ${SAST_PROJECT_DIR}/dvja_jfrog_sast.sarif || echo "No SARIF output found."
        '''
      }
    }
  }
  ...
  post {
    always {
      archiveArtifacts artifacts: '**/dvja_jfrog_sast.sarif', fingerprint: true
      echo "📄 SARIF file archived as 'dvja_jfrog_sast.sarif'"
    }
  }
}
